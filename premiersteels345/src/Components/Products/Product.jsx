...
const rows = sample.map((item, index) => createData(index + 1, ...item));
      ...
      {columns.map((column) => (
        ...
        >
          {column.dataKey === 'Image' ? (
            <img src={row[column.dataKey]} alt={row.Name} style={{ width: '100px', height: 'auto' }} />
          ) : (
            row[column.dataKey]
          )}
        </TableCell>
